          ...
          <form onSubmit={handleAuthSubmit} className="p-6 space-y-4">
            {authMode === 'signup' && (
              <div className="space-y-1.5">
                <label className="block text-sm font-bold text-slate-700">이름 (실명) *</label>
                <input
                  type="text"
                  placeholder="홍길동"
                  value={fullName}
                  onChange={(e) => setFullName(e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-primary text-sm font-medium"
                  required
                />
              </div>
            )}
            ...
            <div className="space-y-1.5">
              <label className="block text-sm font-bold text-slate-700">이메일 주소 *</label>
              <input
                type="email"
            ...
            {authMode === 'signup' && (
              <div className="bg-amber-50/50 border border-amber-200/60 p-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">관리자 계정으로 등록</span>
                </div>
                <input
                  type="checkbox"
                  checked={isAdmin}
            ...
            <button
              type="submit"
              className="w-full py-3 bg-brand-secondary hover:bg-brand-primary text-white font-bold text-sm tracking-widest shadow-md hover:shadow active:scale-95 transition-all duration-200 mt-2 cursor-pointer"
            >
            ...
            <div className="pt-3 border-t border-slate-100 text-center select-none">
              ...
              <button
                ...
                className="text-brand-secondary hover:text-brand-primary text-xs font-bold underline cursor-pointer ml-1"
              >
